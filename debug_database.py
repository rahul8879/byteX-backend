...
# Get database URL from environment variables, default to local sqlite
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./rbyte_ai.db")
...
def check_database_tables():
    ...
    try:
        ...
        if DATABASE_URL.startswith("sqlite"):
            # SQLite: use sqlite_master to list tables
        ...
        for table_name in tables:
            try:
                ...
                results[table_name] = {"count": count, "sample": sample}
            except Exception as inner:
                results[table_name] = {"error": str(inner)}
